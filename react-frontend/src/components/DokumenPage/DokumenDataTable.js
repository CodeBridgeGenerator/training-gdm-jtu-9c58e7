...
const DokumenDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.url}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "dokumen"});
...
<Column field="url" header="url" body={pTemplate1} filter={selectedFilterFields.includes("url")} hidden={selectedHideFields?.includes("url")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Dokumen Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="dokumen"            
          onUploadComplete={() => {
      ...
      <Dialog header="Search Dokumen" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default DokumenDataTable;