...
const CalonCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            pengguna: _entity?.pengguna,latihan: _entity?.latihan,status: _entity?.status,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Calon created successfully" });
