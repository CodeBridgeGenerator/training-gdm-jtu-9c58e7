...
import latihanFakerFactory from "./latihanFakerFactory";
...
const LatihanFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = latihanFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("latihan").create(elem));
...
export default connect(mapState, mapDispatch)(LatihanFakerDialogComponent);
