...
const KelulusanCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            latihan: _entity?.latihan,pelulus: _entity?.pelulus,status: _entity?.status,komen: _entity?.komen,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Kelulusan created successfully" });
