    ...
    return (
        <div className="flex flex-col min-h-screen align-items-center justify-content-center background-image">
                    ...
                    <div className="w-full px-3 mx-3 card md:px-7">
                        ...
                        <div>
                            <div className="w-full mb-4">
                                ...
                                <InputText type="text" placeholder="Enter your registered email" value={email} onChange={(e) => setEmail(e.target.value)} className={classNames(emailError ? 'p-invalid' : '', 'w-full')} onKeyDown={onEnter} />
                                <small className="p-error">{emailError}</small>
                            </div>
                            <div className="w-full mb-4">
                                <p className="mb-1 text-sm">Password</p>
                                <span className="w-full p-input-icon-right">
                                    <i className={`pi ${maskPassword ? 'pi-eye' : 'pi-eye-slash'} cursor-pointer`} onMouseDown={() => setMaskPassword(false)} onMouseUp={() => setMaskPassword(true)} onMouseLeave={() => setMaskPassword(true)} title={`${maskPassword ? 'Show' : 'Hide'} password`} />
                                    <InputText type={maskPassword ? 'password' : 'text'} placeholder="Enter your password" value={password} onChange={(e) => setPassword(e.target.value)} className={classNames(passwordError ? 'p-invalid' : '', 'w-full')} onKeyDown={onEnter} />
                                </span>
                            ...
                            <div className="flex justify-between">
                                ...
                                <p className="text-sm font-semibold cursor-pointer text-primary" onClick={() => setForgotPassword(true)}>
                                    Forgot your password?
                                </p>
                            </div>
                        </div>
                        <div className="mt-7">
                            <Button label="Sign in" className="w-full !rounded-full py-3 text-[16px]" onClick={login} loading={loading}></Button>
                        </div>
                        <div className="mt-4 text-center">
                            <p className="text-sm">
                                Haven't activate your account yet?{' '}
                                <Link to="/signup" className="font-semibold cursor-pointer text-primary">
                                    Set up now
                                </Link>
                            </p>
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                            <Link to="/login-faq" className="m-0 text-sm font-semibold cursor-pointer !text-link">
                                Can't log in?
                            </Link>
                            <p className="m-0 text-sm font-semibold cursor-pointer text-link">Sign in with custom domain</p>
                        </div>
                    </div>
                )}
                {showForgotPassword && !isSend && (
                    <div className="w-full px-3 mx-3 card md:px-7">
                        ...
                        <div className="mt-6 text-center">
                            ...
                            <p className="text-[14px]">Please enter your registered email and we'll send you instructions to reset your password.</p>
                        </div>
                        <div className="mt-6">
                            <div className="w-full mb-4">
                                <p className="mb-1">Email</p>
                                <InputText className="w-full" type="text" placeholder="Enter your registered email" value={email} onChange={(e) => setEmail(e.target.value)}></InputText>
                                <small className="p-error">{verificationError}</small>
                            </div>
                            <div className="mt-6">
                                <Button label="Send reset instructions" className="w-full !rounded-full py-3 text-[16px]" loading={loading} disabled={!email} onClick={sendToForgotResetPage}></Button>
                            </div>
                            <div className="mt-4 text-center">
                                <p className="font-semibold cursor-pointer text-primary" onClick={() => setForgotPassword(false)}>
                                    Back to login
                                </p>
                            </div>
                        </div>
                ...
                {isSend && (
                    <div className="w-full px-3 mx-3 card md:px-7">
                        <div className="my-4 text-center">
                            <img src={emailSent} alt="emailSent" className="w-[60%] min-w-[150px]" />
                        </div>
                        <div className="text-center">
                            <h2 className="mb-4 text-2xl font-semibold">Check your email</h2>
                            <p className="text-[14px]">We've sent you an email with instructions to reset your password. Check your Junk/Span folder if it doesn't arrive. If you still can't log in, click resend email or contact your administrator.</p>
                        </div>
                        <div className="mt-6">
                            <Button
                                label="Back to login"
                        ...
                        <div className="mt-4 text-center">
                            <p className="font-semibold cursor-pointer text-primary" onClick={resendEmail}>
                                Resend email
                            </p>
                        </div>
                    </div>
            ...
            <div className="absolute bottom-0 left-0 flex justify-between w-full px-10 py-10 z-[-1]">
                <img src={illustration1} alt="illustration1" className="w-20rem min-w-[150px]" />
                <img src={illustration2} alt="illustration2" className="w-20rem min-w-[150px]" />
            </div>
            <AppFooter />
        </div>
    );
